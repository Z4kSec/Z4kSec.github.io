...
In addition to the obtained TGT, the PKINIT protocol allows to retrieve the NT hash of the requesting user. This technic is identified as "THEFT5" within the "Certified Pre-Owned" paper and was first published on twitter by Benjamin Delpy (@gentilkiwi). This offensive abuse is due to a fallback feature implemented within the PKINIT protocol. From this build-in feature, the LM-NT hashes can be retrieved from the obtained TGT. The aim of it was to allow the user to authenticate on systems or application which only support legacy authentication.
